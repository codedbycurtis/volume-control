    ...
    browserTabs.forEach(tabData => {
        ...
        slider.addEventListener('input', async () => {
            ...
            if (tabData.isControllerInjected) {
                chrome.tabs.sendMessage(tabData.tab.id, {
                    type: 'setVolume',
                    value: slider.value
                });
                return;
            ...
            let scriptInjection = {
                files: ['js/volume-controller.js'],
                injectImmediately: true,
                target: injectionTarget,
            };
            chrome.scripting
                .executeScript(scriptInjection)
                .then(() => {
                    tabData.isControllerInjected = true;
                    console.log('[Volume Control Extension] Script injection successful');
                })
                .catch(reason => { console.error(`[Volume Control Extension] An error occurred while injecting 'volume-controller.js'.\n${reason}`); });
        });
